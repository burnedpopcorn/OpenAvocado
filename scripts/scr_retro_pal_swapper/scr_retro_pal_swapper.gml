// this is a modified version of RPS
// idk why they changed some parts of the code
// honestly latest stock has smaller code, sooooo why?
...
function pal_swap_set_layer(_pal_sprite, _pal_index, _layer_index, _pal_is_surface) {
    var _data = global.retro_pal_swapper.layer_map[? _layer_index];
    if (_data == undefined) exit;
    
    global.retro_pal_swapper.layer_map[? _layer_index] = {
        sprite: _pal_sprite,
        index: _pal_index,
        is_surf: _pal_is_surface
    };
}
...
function pal_swap_enable_layer(_layer_index) {
    if (!layer_exists(_layer_index)) exit;
